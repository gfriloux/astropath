# PROCEDURE_PLANS.md — Procédure de planification d'astropath
...
- **scope** : `query`, `model`, `view`, `nix`, `notmuch`, `ui`, ou le module touché.
...
La couche `query` + `model` est déterministe : une fixture notmuch (JSON figé) produit
un modèle exact. On stocke ce modèle attendu en **fichier de référence** :

```
tests/fixtures/<cas>/        ← sortie notmuch figée (search/show --format=json)
...
|---|---|---|
| Nouvelle requête / smart folder | `query` `model` | 1. fixture+golden (`test(model): …`) → 2. impl (`feat(query): …`) → 3. doc |
| Nouvel état de fil / mutation de tag | `model` | 1. fixture+golden → 2. impl (`feat(model): …`) → 3. doc DESIGN si invariant touché |
...
**Type :** [requête | état/tag | vue | bug | refactor | doc]
**Objectif :** ...
**Pourquoi :** ...
